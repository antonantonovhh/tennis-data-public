...
def _clv_detail(rows, limit: int = 200) -> str:
    ...
    for r in rows:
        ...
        # Стрелка приглушена намеренно: она показывает движение СЫРОЙ цены,
        # а вердикт выносит CLV. Раньше зелёная стрелка стояла рядом с
        # красным CLV и читалась как противоречие: обыграть цену закрытия
        # мало, обыгрывать надо справедливую, из которой убрана маржа.
        mark = ('<span class="dim">=</span>' if abs(r["move"]) < 0.005
                else f'<span class="dim">{"↑" if r["move"] > 0 else "↓"}</span>')
        body.append([
            f'{e(r["p1"])} — {e(r["p2"])}<br>'
            ...
            f'<span class="num dim">{r["fair"]:.3f}</span>',
    ...
    return ("<h3>Ставка за ставкой</h3>"
            + '<p class="note">«Справедливая» — цена закрытия, из которой '
              'убрана маржа букмекера. Сравнивать надо именно с ней, а не с '
              'самим закрытием: обыграть цену 2.650 при справедливых 2.739 — '
              'значит всё равно заплатить комиссию, поэтому CLV бывает '
              'отрицательным даже когда наша цена выше закрывающей. '
              'Стрелка показывает движение сырой цены и вердиктом не '
              'является.</p>'
            + table(["Матч", "Ставка", "Взяли", "Закрытие", "Справедливая",
                    "CLV"], body)
            + tail)
...
def view_method(q) -> str:
    ...
    return "".join([
        ...
        'самый быстрый измеритель, ему не нужен исход матча, и он вынесен '
        'на отдельную вкладку «Закрытие»; '
...
def view_clv(q) -> str:
    """Вкладка «Закрытие»: CLV сводкой и построчно.

    Вынесена из «Метода» отдельно: там она была первым блоком, но список
    ставок длинный, и из-за него приходилось прокручивать мимо него всё
    остальное — калибровку, смесь и правило отбора.
    """
    return _clv_block()
